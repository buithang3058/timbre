...
Google Passage Indexing (active since 2021) ranks individual passages independently. Pairs directly with GEO citability.

### Rules for Passage-Optimized Content
1. **Self-contained sections**: Each H2 block fully answers one question without requiring context from other sections
2. **Optimal passage length**: 100-200 words per block (sweet spot for both passage indexing and AI citation)
3. **Question-answer structure**: Question-phrased H2/H3 followed by a direct answer in the first sentence
4. **No pronoun-heavy openings**: Start sections with the full subject, not "It" or "This"
5. **Speakable schema**: Add `speakable` CSS selectors for top answer passages
...
- `/brand-seo [project]` — build entity foundation before GEO (prerequisite at score < 6)
- `/seo-aeo [project]` — optimize for Featured Snippets + Knowledge Panel (complements GEO)
- `/schema [project]` — implement structured data for AI discoverability
- `/seo-audit [project]` — technical + on-page audit (prerequisite for top-10 ranking, which feeds GEO)
